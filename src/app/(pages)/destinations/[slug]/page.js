...
async function fetchDestinationData(slug) {
  ...
  try {
    ...
    const response = await fetch(`${process.env.API_URL}/apihome/destination/${slug}`, {
      cache: 'force-cache',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to fetch destination ${slug}: ${response.status} ${response.statusText}`);
...
export async function generateMetadata({ params }) {
  ...
  const description = destination.description || 'Explore this amazing destination.'; // Use specific meta description
  const imageUrl = destination.displayImg ? process.env.NEXT_PUBLIC_URL_PREFIX + destination.displayImg : null; // Construct image URL
...
export async function generateStaticParams() {
  try {
    const response = await fetch(`${process.env.API_URL}/apihome/slugs/destination`, {
       headers: {
    ...
    if (!response.ok) {
       console.error(`Failed to fetch destination slugs: ${response.status} ${response.statusText}`);
       return []; 
    }
...
export default async function DestinationPage({ params }) {
  ...
  return (
    <main>
      ...
      {destinationData.heroImg && <DestinationsHero imgUrl={`${destinationData.heroImg}`} title={destinationData.title} description={destinationData.description} />}

      {destinationData.highlight && (
        <Highlights {...destinationData.highlight} url={`/contact?src=${resolvedParams.slug}`} />
      )}
      ...
      {destinationData.experiences && destinationData.experiences.length > 2 && (
      ...
      <Banner
        title={destinationData.bannerTitle || "Ready to Explore?\nLet's Plan Your Trip!"}
