...
async function fetchExperienceData(slug) {
  try {
    const response = await fetch(`${process.env.API_URL}/apihome/experience/${slug}`, {
    ...
    if (!response.ok) {
      console.error(`Failed to fetch experience data for ${slug}: ${response.status} ${response.statusText}`);
...
export async function generateMetadata({ params }) {
  ...
  if (!experienceData) {
    return {
      ...
      description: 'Information for the requested experience type could not be found.',
    };
  }
...
export async function generateStaticParams() {
  try {
    ...
    const response = await fetch(`${process.env.API_URL}/apihome/slugs/experience`, {
       headers: {
         'Authorization': `Bearer ${process.env.API_TOKEN}` // If auth is needed
       }
    });
    ...
    if (!response.ok) {
       console.error(`Failed to fetch experience slugs: ${response.status} ${response.statusText}`);
       return [];
    }
...
export default async function ExperiencePage({ params }) {
  ...
  return (
    <main>
      ...
      {experienceData.destinations && experienceData.destinations.length > 0 && (
        <Destinations
