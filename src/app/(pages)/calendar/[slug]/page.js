...
async function fetchCalendarData(slug) {
  try {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_URL_PREFIX;
    const apiToken = process.env.NEXT_PUBLIC_TOKEN;

    if (!apiUrl || !apiToken) {
        console.error("API URL or Token is not defined in environment variables for client-side fetching.");
    ...
    if (!response.ok) {
      console.error(`Failed to fetch calendar data for ${slug}: ${response.status} ${response.statusText}`);
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        console.error("Failed to load calendar data, triggering Not Found:", err);
  ...
  const monthName = slug ? slug.charAt(0).toUpperCase() + slug.slice(1) : 'Month';
  ...
  return (
    <main>
      ...
      {heroImageUrl && (
        <MonthHero imgUrl={heroImageUrl} month={monthName} />
      )}

      {/* Highlights Section - Conditionally render */}
      {calendarData.highlight && (
        <Highlights {...calendarData.highlight} />
      )}
      ...
      {calendarData.tours && calendarData.tours.length > 0 && (
        <Tours
          heading={defaultToursHeading}
          tours={calendarData.tours}
        />
      )}
