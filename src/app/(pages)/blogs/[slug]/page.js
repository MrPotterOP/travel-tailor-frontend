...
async function fetchBlogData(slug) {
    ...
    try {
      const response = await fetch(`${process.env.API_URL}/apihome/blog/${slug}`, {
        cache: 'force-cache',
        headers: {
...
function generateKeywords(title, description) {
  ...
  const commonWords = new Set(['a', 'an', 'the', 'is', 'in', 'it', 'of', 'for', 'on', 'with', 'to', 'and', 'when', 'visiting']);
...
export async function generateMetadata({ params }) {
    ...
    const shareImageUrl = data.seo?.shareImage?.startsWith('/')
        ...
        : data.seo?.shareImage || // Handle if it's already absolute
          (data.displayImg?.startsWith('/') ? `${process.env.BACK_URL_PREFIX}${data.displayImg}` : data.displayImg);
    ...
    return {
        ...
        // --- Basic Meta Tags ---
        keywords: data.seo?.keywords || generateKeywords(data.title, data.description),
...
async function BlogPage({params}) {
    ...
    return ( 
        <article>
          ...
          {data.displayImg && <BlogHero imgUrl={data.displayImg} title={data.title} description={data.description} auther={data.author} date={data.createdAt} />}
          {data.body && <BlogBody body={data.body} />}

          <aside>
            { (data.blogs.length  > 0) && <Blogs blogs={data.blogs} heading={{title: "Similar /sPosts\\s", description: "You might like these blogs"}} />}
            { (data.tours.length > 0) && <Tours tours={data.tours} heading={{title: "/sTours\\s you might /n enjoy", description: "Explore the world with our curated tours"}} />}
            <Banner
