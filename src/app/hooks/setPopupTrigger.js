...
export const usePopupTrigger = () => {
  ...
  useEffect(() => {
    ...
    let status = localStorage.getItem(FORM_STATUS_KEY);


    if (!status) {
      status = STATUS.INITIAL;
      localStorage.setItem(FORM_STATUS_KEY, status);
    }
  ...
  const handleClose = useCallback(() => {
    ...
    const currentStatus = localStorage.getItem(FORM_STATUS_KEY);
    let nextStatus = '';
    ...
    if (nextStatus) {
      localStorage.setItem(FORM_STATUS_KEY, nextStatus);
    }
    setIsVisible(false);
  ...
  const handleSuccess = useCallback(() => {
    if (typeof window === 'undefined') return;
    localStorage.setItem(FORM_STATUS_KEY, STATUS.DONE);
    setIsVisible(false);
  }, []);
